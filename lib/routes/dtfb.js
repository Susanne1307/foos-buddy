...
async function getTournamentDates() {
  ...
  const reg = new RegExp(/(3[01]|[12][0-9]|0?[1-9])\.(1[012]|0?[1-9])\.\s/g);
...
async function getTournamentNames() {
  const result = await fetch('https://dtfb.de/index.php/termine');
  const html = await result.text();
  const reg = new RegExp(/(?<=wrap>\s)([^<]+)(?=\s<[br|/td])/g);
  ...
  cleanedTournamentNames.splice(0, 4);
...
async function getPlayerImages() {
  ...
  const html = await result.text();
  const reg = new RegExp(/\bhttps?:[^)''"]+\.(?:jpg|jpeg|gif|png)/gi);
